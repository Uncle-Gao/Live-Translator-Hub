...
const DictRow = ({ label, info }) => (
  <div className="flex items-center justify-between">
    ...
    {info?.exists && (
      <span className="text-[11px] text-emerald-400/50">{info.count} 条</span>
    )}
    {!info?.exists && (
      <span className="text-[11px] text-amber-400/40">不存在（内置字典将用于部署）</span>
    )}
  </div>
);
    ...
    <div className="bg-[#1A1C1E] border border-white/5 rounded-3xl p-6 space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          <DictRow label={`${lang}.json (主字典)`} info={mainDict} />
          <DictRow label="Localizable.strings.zh-CN (原生字符串)" info={stringsDict} />
          <p className="text-[10px] text-white/20 italic pt-1">
            Claude 使用内置字典文件，部署时自动注入，无需手动生成。
          </p>
        </div>
      )}
    </div>
