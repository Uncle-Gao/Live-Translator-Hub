...
│   │   ├── electron/main.js        # Proceso principal, canal IPC y persistencia de configuración
...
Gestiona el estado de implementación de la sinización, la versión del diccionario y las reglas de bloqueo para Cursor y Claude desde la misma interfaz, sin necesidad de cambiar de herramienta.
...
| **API Keys** | Gestionar claves API para múltiples motores de traducción con IA (compatible con OpenAI, Anthropic, Google Gemini, DeepL), almacenadas cifradas mediante `safeStorage` de Electron |
...
- **Almacenamiento cifrado de claves API**: Se guardan cifradas mediante `safeStorage` de Electron en `~/.live_translator_hub/api_keys.enc`, sin escribirse en archivos de configuración
