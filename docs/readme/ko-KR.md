...
Live Translator Hub는 **Electron + React GUI 데스크톱 애플리케이션**으로, Cursor와 Claude 두 가지 AI 프로그래밍 도구에 원클릭 한글화를 제공합니다. 통합된 번역 런타임 커널을 통해 하나의 인터페이스에서 두 대상 애플리케이션의 엔진 배포, API 키 구성 및 사전 생성을 관리합니다.
...
`packages/core/src/translator-engine.js`는 대상 애플리케이션에 주입되는 유일한 런타임입니다. 순수 브라우저 JS이며 모듈 의존성이 없습니다. 책임은 다음과 같습니다:

- **사전 매칭**: 정적 단어 + 정규식 패턴
- **AI 번역 프록시 브릿지**: Webview 환경에서 `postMessage`를 통해 번역 요청을 메인 창으로 전달하여 CSP 네트워크 제한 우회
...
Translation Bridge 아키텍처를 통해 AI 번역 기능이 메인 창에서 모든 계층의 Webview 플러그인(예: Claude Code)까지 관통할 수 있어, 엄격한 CSP 정책 하에서의 네트워크 차단 문제를 해결합니다.

### 4개 패널 기능 레이아웃

| 패널 | 기능 |
| :--- | :--- |
| **Cursor Engine** | Cursor 한글화 배포/복원, 메인 창 및 Webview 플러그인의 영역별 차단 규칙 관리 |
| **Claude Engine** | Claude 한글화 배포/복원, 건너뛰기 규칙 구성 |
...
- `Cmd + Option + Shift + B` (Mac) / `Ctrl + Alt + Shift + B` (Win)로 파란색 점선 강조 테두리 전환
- 강조 모드에서 `Option` (Mac) / `Alt` (Win)을 누른 상태로 중국어 위에 마우스를 올리면 원문 확인 가능

### 영역별 차단 규칙

각 엔티티(메인 창 및 각 플러그인)는 완전히 독립적인 차단 규칙 집합(CSS 선택자, URL 매칭, 제목 매칭)을 가지므로 코드 영역과 핵심 상호작용 영역이 번역의 영향을 받지 않습니다.
...
- macOS 13+ (권장)
- Node.js 18+
- Cursor 또는 Claude 데스크톱 애플리케이션 설치
