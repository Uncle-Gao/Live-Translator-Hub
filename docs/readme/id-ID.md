...
│   │   ├── electron/main.js        # Proses utama, saluran IPC & persistensi konfigurasi
...
`packages/core/src/translator-engine.js` adalah satu-satunya runtime yang disuntikkan ke dalam aplikasi target — murni JS browser, tanpa dependensi modul. Tanggung jawabnya meliputi:
...
- **Cache Terjemahan**: Cache persisten berbasis `localStorage`, dengan kunci `live_i18n_cache_<nama_entitas>`
...
Kelola status penerapan sinisasi, versi kamus, dan aturan pemblokiran untuk Cursor dan Claude secara terpisah dalam satu antarmuka, tanpa perlu beralih alat.

### Penetrasi Webview di Semua Skenario

Melalui arsitektur Translation Bridge, kemampuan terjemahan AI dapat menembus dari jendela utama ke semua lapisan plugin Webview (misalnya Claude Code), mengatasi masalah pemblokiran jaringan di bawah kebijakan CSP yang ketat.

### Tata Letak Empat Panel

| Panel | Fungsi |
| :--- | :--- |
| **Cursor Engine** | Terapkan/pulihkan sinisasi Cursor, kelola aturan pemblokiran terpisah untuk jendela utama & plugin Webview |
| **Claude Engine** | Terapkan/pulihkan sinisasi Claude, konfigurasikan aturan lewati |
| **API Keys** | Kelola kunci API untuk beberapa mesin terjemahan AI (mendukung OpenAI, Anthropic, Google Gemini, DeepL), kunci disimpan terenkripsi melalui `safeStorage` Electron |
| **Dict Generator** | Ekstrak string UI dari kode sumber aplikasi target, hasilkan kamus terjemahan secara batch melalui AI |

### Debugging Interaktif

- `Cmd + Option + Shift + B` (Mac) / `Ctrl + Alt + Shift + B` (Win) untuk mengalihkan batas sorotan garis putus-putus biru
- Dalam mode sorotan, tahan `Option` (Mac) / `Alt` (Win) dan arahkan kursor ke teks Mandarin untuk melihat teks asli

### Aturan Pemblokiran Terpisah

Setiap entitas (jendela utama & setiap plugin) memiliki kumpulan aturan pemblokiran yang sepenuhnya independen (pemilih CSS, pencocokan URL, pencocokan judul), memastikan area kode & area interaksi inti tidak terpengaruh oleh terjemahan.
...
3. Klik **Deploy** untuk menerapkan sinisasi satu-klik
...
- **Penyimpanan Kunci API Terenkripsi**: Disimpan terenkripsi melalui `safeStorage` Electron ke `~/.live_translator_hub/api_keys.enc`, tidak ditulis ke file konfigurasi
...
*Proyek ini hanya untuk tujuan pembelajaran dan pertukaran. Kualitas terjemahan dipengaruhi oleh model AI yang dipilih.*
