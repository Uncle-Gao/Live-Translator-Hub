...
> GUI full-scene real-time localization engine covering Cursor and Claude desktop apps, supporting Webview plugin penetration and AI asynchronous translation.
...
│   │   ├── electron/main.js        # Main process, IPC channels, and config persistence
...
`packages/core/src/translator-engine.js` is the only runtime injected into the target applications — pure browser JS with no module dependencies. Responsibilities include:
...
Manage the localization deployment status, dictionary versions, and blocking rules for Cursor and Claude respectively within the same interface, without switching tools.
